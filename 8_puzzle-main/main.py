    ...
    def get_neighbors(self, state):
        ...
        for dr, dc in directions:
            ...
            if 0 <= new_row < 3 and 0 <= new_col < 3:
                ...
                move = tuple(map(tuple, new_state))
                moves.append(move)
        return moves
...
def initial_state_selector(goal_state):
    ...
    try:
        background = pygame.image.load(r"D:\Downloads\8_puzzle-main (1)\asset\anh_backgound\map3.png")
    ...
    while running:
        ...
        # Tiêu đề
        render_text_with_border(screen, "8-Puzzle Solver", title_font, (255, 105, 255), (0, 0, 0), (WIDTH // 2, 50))        # Vẽ lưới trạng thái ban đầu
        render_text_with_border(screen, "Initial State", label_font, (255, 255, 0), (0, 0, 0),
        ...
        render_text_with_border(screen, "Goal State", label_font, (255, 255, 0), (0, 0, 0),
                                (goal_offset_x + 150, goal_offset_y - 60))
        draw_grid(goal_state, goal_offset_x, goal_offset_y, tile_size)

        # Vẽ các nút

        render_text_with_border(screen, "->", label_font, (255, 255, 0), (0, 0, 0),
                                (button_right_rect.centerx, button_right_rect.centery ))
        render_text_with_border(screen, "Confirm", label_font, (255, 255, 0), (0, 0, 0),
...
def main_game(initial_state, goal_state):
    ...
    group_font = pygame.font.SysFont("Comic Sans MS", 25, bold=True)

    puzzle = EightPuzzle(initial_state, goal_state)

    # Grid positions and sizes
    tile_size = 80
    algo_grid_x = WIDTH // 2 - tile_size * 3 - 100
    algo_grid_y = 120
    initial_grid_x = WIDTH // 2 + 40
    initial_grid_y = 120
    goal_grid_x = initial_grid_x + tile_size * 3 + 40
    goal_grid_y = 120

    # Button layout setup for 6 rows
    button_width, button_height = 100, 30
    button_spacing_x, button_spacing_y = 110, 40
    buttons_per_row = 4
    total_button_width = buttons_per_row * button_width + (buttons_per_row - 1) * (button_spacing_x - button_width)
    start_x = (WIDTH - total_button_width) // 2
    start_y = algo_grid_y + tile_size * 3 + 30

    # Define buttons with new groupings
    buttons = [
        # Row 1: Uninformed Search Algorithms (BFS, DFS, UCS)
        ...
        # Row 2: Informed Search Algorithms (GBFS, A*, IDA*)
        ...
        # Row 3: Local Optimization Algorithms (SHC, SAHC, RHC, SA)
        ("SHC", pygame.Rect(start_x, start_y + 2 * button_spacing_y, button_width, button_height)),
        ("SAHC", pygame.Rect(start_x + button_spacing_x, start_y + 2 * button_spacing_y, button_width, button_height)),
        ("RHC", pygame.Rect(start_x + 2 * button_spacing_x, start_y + 2 * button_spacing_y, button_width, button_height)),
        ("SAS", pygame.Rect(start_x + 3 * button_spacing_x, start_y + 2 * button_spacing_y, button_width, button_height)),
        ("Beam", pygame.Rect(start_x, start_y + 3 * button_spacing_y, button_width, button_height)),


        # Row 4: Local Optimization (BS, GA) + Search in Complex Environments (AND-OR, Belief)
        ("Genetic", pygame.Rect(start_x + button_spacing_x, start_y + 3 * button_spacing_y, button_width, button_height)),
        ("AND-OR", pygame.Rect(start_x + 2 * button_spacing_x, start_y + 3 * button_spacing_y, button_width, button_height)),
        ("Belief", pygame.Rect(start_x + 3 * button_spacing_x, start_y + 3 * button_spacing_y, button_width, button_height)),

        # Row 5: Search in Complex Environments (POS) + Constraint Satisfaction (Backtracking, Forward, Min-conf)
        ("POS", pygame.Rect(start_x, start_y + 4 * button_spacing_y, button_width, button_height)),
        ("Backtrack", pygame.Rect(start_x + button_spacing_x, start_y + 4 * button_spacing_y, button_width, button_height)),
        ("Forward", pygame.Rect(start_x + 2 * button_spacing_x, start_y + 4 * button_spacing_y, button_width, button_height)),
        ("MinConf", pygame.Rect(start_x + 3 * button_spacing_x, start_y + 4 * button_spacing_y, button_width, button_height)),

        # Row 6: Reinforcement Learning (Q-Learning)
        ("QLearn", pygame.Rect(start_x, start_y + 5 * button_spacing_y, button_width, button_height)),
    ]

    # Define group labels and their positions
    group_labels = [
    ("Uni", start_y - 20),
    ("Informed", start_y + button_spacing_y - 20),
    ("Local", start_y + 2 * button_spacing_y - 20),
    ("Optimization", start_y + 3 * button_spacing_y - 20),
    ("Complex", start_y + 4 * button_spacing_y - 20),
    ("Learning", start_y + 5 * button_spacing_y - 20),
]

    # Other buttons (BACK, RESET, VIEW, INFO)
    ...
    info_button_rect = pygame.Rect(WIDTH - 450, HEIGHT - 70, 100, 40)

    # Performance history
    performance_history = {
        "BFS": [], "DFS": [], "UCS": [], "Greedy": [], "A*": [], "IDA*": [],
        "SHC": [], "SAHC": [], "RHC": [], "SAS": [], "Beam": [], "Genetic": [], 
        "AND-OR": [], "Belief": [], "POS": [], "Backtrack": [], "Forward": [], 
        "MinConf": [], "QLearn": []
    }
    ...
    while running:
        if needs_redraw or (solution and solution_index < len(solution)):
            ...
            render_text_with_border(screen, "Initial State", label_font, (0, 255, 0), (0, 0, 0),
                                    (initial_grid_x + (tile_size * 3) // 2, initial_grid_y - 20))
            draw_grid(initial_state, initial_grid_x, initial_grid_y, tile_size)

            render_text_with_border(screen, "Goal State", label_font, (0, 255, 0), (0, 0, 0),
                                    (goal_grid_x + (tile_size * 3) // 2, goal_grid_y - 20))
            draw_grid(goal_state, goal_grid_x, goal_grid_y, tile_size)

            # Display algorithm state
            if solution:
                if solution_index < len(solution):
                    draw_grid(solution[solution_index], algo_grid_x, algo_grid_y, tile_size)
                    solution_index += 1
                    pygame.time.wait(400)
                else:
                    draw_grid(solution[-1], algo_grid_x, algo_grid_y, tile_size)
            else:
                draw_grid(display_state, algo_grid_x, algo_grid_y, tile_size)
            ...
            # Draw group labels
            for label, y_pos in group_labels:
                render_text_with_border(screen, label, group_font, (255, 165, 0), (0, 0, 0), (start_x - 80, y_pos))
            ...
            for label, rect in buttons:
                ...
                text_surface = button_font.render(label, True, (255, 255, 0))
            ...
            text_surface = button_font.render("INFO", True, (255, 255, 0))
            text_rect = text_surface.get_rect(center=(info_button_rect.centerx, info_button_rect.centery))
            screen.blit(text_surface, text_rect)
            ...
            text_surface = button_font.render("VIEW", True, (255, 255, 0))
            text_rect = text_surface.get_rect(center=(view_button_rect.centerx, view_button_rect.centery))
            screen.blit(text_surface, text_rect)
            ...
            text_surface = button_font.render("RESET", True, (255, 255, 0))
            text_rect = text_surface.get_rect(center=(reset_button_rect.centerx, reset_button_rect.centery))
            screen.blit(text_surface, text_rect)
            ...
            text_surface = button_font.render("Back", True, (255, 255, 0))
            text_rect = text_surface.get_rect(center=(back_button_rect.centerx, back_button_rect.centery))
            screen.blit(text_surface, text_rect)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if info_button_rect.collidepoint(mouse_pos):
                    ...
                    display_algorithm_info(performance_history)
                    import platform
                    import os
                ...
                for label, rect in buttons:
                    if rect.collidepoint(mouse_pos):
                        ...
                        if label == "MinConf":
                        ...
                        elif label in ["BFS", "DFS", "UCS", "Greedy", "A*", "IDA*", "SHC", "SAHC", "RHC",
                            ...
                            if result == "BACK":
                                ...
                                render_text_with_border(screen, "8-Puzzle Solver", title_font, (255, 0, 0), (0, 0, 0),
                                                        (WIDTH // 2, 50))
                                render_text_with_border(screen, "Initial State", label_font, (0, 255, 0), (0, 0, 0),
                                                        (initial_grid_x + (tile_size * 3) // 2,
                                                         initial_grid_y - 20))
                                draw_grid(initial_state, initial_grid_x, initial_grid_y, tile_size)
                                render_text_with_border(screen, "Goal State", label_font, (0, 255, 0), (0, 0, 0),
                                                        (goal_grid_x + (tile_size * 3) // 2, goal_grid_y - 20))
                                draw_grid(goal_state, goal_grid_x, goal_grid_y, tile_size)
                                draw_grid(initial_state, algo_grid_x, algo_grid_y, tile_size)
                                for label, rect in buttons:
                                    corner_radius = 10
                                    button_color = (255, 255, 0) if selected_button == label else (26, 125, 255)
                                    pygame.draw.rect(screen, button_color, rect, border_radius=corner_radius)
...
def belief_state_interface(initial_belief, goal_state, algorithm_type, performance_history):
    ...
    button_run_rect = pygame.Rect(50, HEIGHT - 70, 100, 40)
    button_back_rect = pygame.Rect(170, HEIGHT - 70, 100, 40)
